		...
		<div className="grow flex flex-row justify-evenly">
			<div className="w-full h-full">
				<div className="flex flex-col items-center justify-center h-full p-4 mx-auto">
					<div className="w-full bg-base-200 rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 border-primary">
						<div className="p-6 space-y-4 md:space-y-6 sm:p-8">
							<h1 className="text-xl font-bold leading-tight tracking-tight md:text-2xl text-center">Sign up for a new account</h1>
							<form className="space-y-4" action="#">
								<InputField name="username" label="Username" type="text" placeholder="" error={errors?.username} value={signupForm.username} onChange={handleSignupFormChange} />
								<InputField name="password" label="Password" type="password" placeholder="" error={errors?.password} value={signupForm.password} onChange={handleSignupFormChange} />
								<InputField name="confirmPassword" label="Confirm Password" type="password" placeholder="" error={errors?.confirmPassword} value={signupForm.confirmPassword} onChange={handleSignupFormChange} />
								{errors?.detail && <p className="text-error text-sm text-center">{errors.detail}</p>}
								<SubmitButton label="Sign up" onClick={submitSignupForm} />
								<p className="text-sm">
									Already have an account ?{" "}
									<NavLink to="/login" className="font-medium link">
										Login
									</NavLink>
								</p>
							</form>
